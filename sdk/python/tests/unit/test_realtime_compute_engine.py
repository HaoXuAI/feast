...
class TestCalculationNode:
    def test_no_calculations_passthrough(self):
        """Test CalculationNode with empty calculations passes data through."""
        from feast.infra.compute_engines.backends.pandas_backend import PandasBackend

        backend = PandasBackend()
        input_table = pa.table({"a": [1, 2, 3], "b": [4.0, 5.0, 6.0]})
        input_node = DataInputNode(name="input", data=input_table)
        ...
        result = calc_node.execute(context)
        # Empty calculations should pass through the input unchanged
        assert result.data.num_rows == 3
        assert result.data.num_columns == 2

    def test_calculation_node_graceful_without_module(self):
        """Test CalculationNode handles missing feast.calculation module gracefully."""
        from feast.infra.compute_engines.backends.pandas_backend import PandasBackend

        backend = PandasBackend()
        input_table = pa.table({"price": [10, 20, 30], "quantity": [2, 3, 4]})
        input_node = DataInputNode(name="input", data=input_table)

        # Use a mock calculation object since the module may not be installed
        mock_calc = MagicMock()
        mock_calc.name = "total"
        mock_calc.expr = "price * quantity"

        calc_node = CalculationNode(
            name="calc",
            calculations=[mock_calc],
            backend=backend,
            inputs=[input_node],
        )
        ...
        # Should not raise even if feast.calculation is not available
        result = calc_node.execute(context)
        # Data should pass through unchanged when module is not available
        assert result.data.num_rows == 3
...
class TestRealTimeComputeEngine:
    ...
    def test_has_calculations_false(self):
        """Test _has_calculations returns False for ODFV without calculations."""
        engine = RealTimeComputeEngine(
            repo_config=MagicMock(),
            online_store=MagicMock(),
        )
        ...
        odfv = _create_odfv()
        assert engine._has_calculations(odfv) is False

    def test_has_calculations_empty_list(self):
        """Test _has_calculations returns False for ODFV with empty calculations list."""
        engine = RealTimeComputeEngine(
            repo_config=MagicMock(),
            online_store=MagicMock(),
        )
    ...
    def test_has_calculations_true(self):
        """Test _has_calculations returns True when calculations exist."""
        engine = RealTimeComputeEngine(
            repo_config=MagicMock(),
            online_store=MagicMock(),
        )
        ...
        mock_calc.name = "total"
        mock_calc.expr = "price * quantity"
        odfv.calculations = [mock_calc]
        assert engine._has_calculations(odfv) is True
    ...
    def test_build_push_execution_plan_with_calculations(self):
        """Test execution plan includes calculation node when calculations exist."""
        engine = RealTimeComputeEngine(
            repo_config=MagicMock(),
            online_store=MagicMock(),
            backend="pandas",
        ...
        mock_calc = MagicMock()
        mock_calc.name = "total"
        mock_calc.expr = "input_a * input_b"
        odfv.calculations = [mock_calc]
        ...
        # Should have: input -> calculations -> transform -> online_write
        assert f"{odfv.name}_input" in node_names
        assert f"{odfv.name}_calculations" in node_names
        assert f"{odfv.name}_transform" in node_names
        assert f"{odfv.name}_online_write" in node_names

        # Verify ordering: calculations before transform
        calc_idx = node_names.index(f"{odfv.name}_calculations")
        transform_idx = node_names.index(f"{odfv.name}_transform")
        assert calc_idx < transform_idx
