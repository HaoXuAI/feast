...
class CalculationNode(LocalNode):
    """
    A DAG node that applies Calculation expressions (SQL-like expressions)
    to input data. This enables declarative feature engineering without
    requiring a Python UDF.

    Calculations are evaluated using expression engines (e.g., sqlglot)
    and support arithmetic, comparisons, CASE/WHEN, COALESCE, etc.

    This node is designed to work alongside ODFVTransformationNode in the
    execution plan. When an ODFV has both calculations and a UDF, the
    calculations are applied first, then the UDF receives the enriched data.

    Usage:
        calculations = [Calculation(name="total", expr="price * quantity")]
        node = CalculationNode("calc", calculations=calculations, backend=backend)
    """
    ...
    def execute(self, context: ExecutionContext) -> ArrowTableValue:
        ...
        # Try to import and use the Calculation module's apply function.
        # This allows the Calculation feature to be developed independently
        # and plugged in when available.
        try:
            from feast.calculation import apply_calculations

            result = apply_calculations(input_table, self.calculations)
            if isinstance(result, pa.Table):
                return ArrowTableValue(data=result)
            else:
                return ArrowTableValue(data=pa.Table.from_pandas(pd.DataFrame(result)))
        except ImportError:
            logger.warning(
                "Calculation module not available. "
                "Install feast with calculation support to use SQL expressions."
            )
            return ArrowTableValue(data=input_table)
...
class RealTimeComputeEngine:
    ...
    Extensibility:
    - CalculationNode: When ODFV has `calculations` (SQL expressions), they are
      applied as a separate DAG node before the UDF transformation. This allows
      the Calculation feature (HaoXuAI/feast#1) to plug in seamlessly.
    - The DAG plan is: DataInput -> [Calculation] -> [UDFTransform] -> [Write]
    """
    ...
    def _get_backend(self) -> DataFrameBackend:
        ...
        return BackendFactory.from_name("pandas")
    ...
    def _has_calculations(self, odfv: "OnDemandFeatureView") -> bool:  # noqa: F821
        """Check if ODFV has Calculation expressions defined."""
        calculations = getattr(odfv, "calculations", None)
    ...
    def _build_push_execution_plan(
        ...
        The plan is: DataInput -> [Calculation] -> [UDFTransform] -> [OnlineWrite, OfflineWrite]

        When both Calculations and a UDF are present, Calculations run first to
        produce intermediate columns, then the UDF receives the enriched data.
        """
        ...
        # Node 2 (optional): Calculation expressions
        if self._has_calculations(odfv):
            calc_node = CalculationNode(
                name=f"{odfv.name}_calculations",
                calculations=odfv.calculations,
                backend=backend,
                inputs=[last_node],
            )
        ...
        # Node 3 (optional): UDF transformation
    ...
    def compute_from_push(
        ...
        transformations (including Calculations), and write results to the
        appropriate stores.
        ...
        for odfv in dependent_odfvs:
            ...
            try:
                ...
                # Extract the transformed result from the last compute node
                # (either transform or calculations, whichever ran last)
                for node_name in [
                    f"{odfv.name}_transform",
                    f"{odfv.name}_calculations",
                ]:
    ...
    def transform(
        ...
        get_online_features). This applies both Calculations and UDF transformations
        without writing results to any store.
        ...
        """
        # Apply calculations first if present
        if self._has_calculations(odfv):
            try:
                from feast.calculation import apply_calculations

                input_df = pd.DataFrame(
                    apply_calculations(input_df.to_dict("list"), odfv.calculations)
                )
            except ImportError:
                logger.warning(
                    "Calculation module not available for ODFV '%s'", odfv.name
                )

        # Then apply UDF transformation
        if self._has_udf(odfv):
    ...
    def transform_dict(
        ...
        Supports both Calculations and UDF transformations.
        ...
        if self._has_calculations(odfv):
            try:
                ...
                feature_dict = apply_calculations(feature_dict, odfv.calculations)
            except ImportError:
                logger.warning(
                    "Calculation module not available for ODFV '%s'", odfv.name
                )

        # Apply UDF transformation via Arrow path for consistent behavior
        if self._has_udf(odfv):
